...
<div id="wrap">
  ...
  <form action="process/purchaseCartProcess.jsp" method="post"><%-- form위치ㅐ!! --%>
  <div id="body">
  ...
  //	int p_no = Integer.parseInt(request.getParameter("p_no"));
	...
	/* 	if(u_coin<(pro.getP_price()*amount)){
		...
		//coin = dao.bringCoin(conn, u_id);
				...
				%>
				 <c:set var="total_coin" value="0"/>
				  <c:set var="list" value="<%=cart%>"/> 
          <table id="table1">
            <tr>
      	      <th width="172">상품정보</th>
            ...
            <tr>   <%--foreach문 시작 --%>
              ...
              <td width="172">${cart_for.p_name}</td>
              <td>${cart_for.p_price}</td>
              <td>${cart_for.amount}</td>
              <td>${cart_for.p_price*cart_for.amount}</td>
            </tr>			<%--foreach문 끝 --%>
         	 <c:set var="total_coin" value="${total_coin+(cart_for.p_price*cart_for.amount)}"/>
             </c:forEach>
   ...
   <c:set var="u_coin" value="<%=u_coin %>"/>
   <c:if test="${u_coin-total_coin < 0}">
		<script>
		    var coin_frm = confirm('보유코인이 부족합니다 ㅠㅠ 코인충전 페이지로 가시겠습니까?');
		    if(coin_frm){
		    	location.href='coin_charge.jsp';
		    }
		    else{
  ...
  <%-- 	<input type="hidden" name="amount" value="<%=amount%>"/>
  	...
  	<input type="hidden" name="total_coin" value="${total_coin}"/>
  	<input type="hidden" name="u_coin" value="${u_coin-total_coin}"/>
